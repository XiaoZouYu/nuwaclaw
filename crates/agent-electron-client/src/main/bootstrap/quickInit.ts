 ...
 *
 * 配置优先级（per-field）: santiclaw.json (quickInit scope) → 环境变量 → 客户端默认值
 *
 * 环境变量:
 ...
 *   NUWAX_WORKSPACE_DIR    → workspaceDir      (可选, 默认 ~/.santiclaw/workspace)
 ...
 *   workspaceDir   → ~/.santiclaw/workspace
 *   username       → ''
 */
 ...
 * 读取快捷初始化配置
 *
 * 优先级（per-field）: santiclaw.json → 环境变量 → 默认值
 * 结果缓存，每次启动只读一次
 */
export function readQuickInitConfig(): QuickInitConfig | null {
  ...
  const filePath = path.join(appDataDir, 'santiclaw.json');

  // --- 读取 santiclaw.json (quickInit scope) ---
