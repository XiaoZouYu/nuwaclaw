...
describe("hasRequiredQuickInitFields", () => {
  ...
  it("should return false when savedKey is missing", () => {
    expect(
      hasRequiredQuickInitFields({ serverHost: "https://www.santisaas.com/" }),
    ).toBe(false);
  });

  it("should return false when serverHost is empty string", () => {
    expect(
      hasRequiredQuickInitFields({ serverHost: "", savedKey: "key-123" }),
    ).toBe(false);
  });
  ...
  it("should return false when fields are wrong type", () => {
    expect(
      hasRequiredQuickInitFields({ serverHost: 123, savedKey: "key" }),
    ).toBe(false);
    expect(
      hasRequiredQuickInitFields({ serverHost: "host", savedKey: 456 }),
    ).toBe(false);
  });
});
